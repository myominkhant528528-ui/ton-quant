...
<svelte:head><title>ဈေးကွာအမြတ် (Arbitrage) — TON Quant</title></svelte:head>

<header class="hd">
  <div class="hd-top"><h1>ဈေးကွာအမြတ် (Arbitrage)</h1>
    <span class="muted">TON DEX ရှိ pool အချင်းချင်း spread · fee (2×0.3%) နှင့် gas နုတ်ပြီး net · တိုက်ရိုက် 60 စက္ကန့်</span>
    {#if scannedAt}<span class="muted upd">scan {new Date(scannedAt).toLocaleTimeString()}</span>{/if}
  </div>
</header>

{#if st === 'loading'}<div class="muted pad">pool များကို scan လုပ်နေသည်…</div>
{:else if st === 'error'}<div class="card bad">အမှား: {err}</div>
{:else}
  <!-- GRAM featured -->
  <section class="card gramc">
    <div class="sec-title">GRAM <span class="muted">· {gram?.sym || 'GRM'} · စာမျက်နှာ၏ အဓိကအာရုံ</span>
      <a class="tlink" href="{base}/token?a={GRAM}">token စာမျက်နှာ →</a></div>
    {#if !gram}
      <p class="muted sm">GRAM တွင် လက်ရှိ liquidity ရှိသော pool နှစ်ခုအောက်သာ ရှိသည် — arbitrage လုပ်စရာ မရှိပါ။ Scan ကို 60 စက္ကန့်တိုင်း ဆက်လုပ်နေသည်။</p>
    {:else}
      <div class="duel">
        <div class="side">
          <span class="sl">ဝယ်ရန် · {gram.buy.dex}</span>
        ...
        <div class="mid">
          ...
          <span class="muted sm">gross spread</span>
          <span class="net mono {netCls(gram.net.netUsd)}">{gram.net.netUsd > 0 ? '+' : ''}{fmtUsd(gram.net.netUsd)} net</span>
          {#if gram.net.size > 0}<span class="muted sm">အကောင်းဆုံးအရွယ်အစား {fmtUsd(gram.net.size)}</span>{/if}
        </div>
        <div class="side">
          <span class="sl">ရောင်းရန် · {gram.sell.dex}</span>
          <span class="sv mono">{fmtUsd(gram.sell.price)}</span>
          <span class="sn muted">{gram.sell.pair} · liq {fmtUsd(gram.sell.liq)}</span>
        </div>
      </div>
      <table class="pt">
        <thead><tr><th>Pool</th><th class="r">ဈေးနှုန်း</th><th class="r">Liquidity</th><th class="r">Vol 24 နာရီ</th><th class="r">vs min</th></tr></thead>
        <tbody>
  ...
  <section class="card tw">
    <div class="sec-title">အခွင့်အလမ်းများ <span class="muted">· snapshot မှ နေ့စဉ် spread အမြင့်ဆုံးများ၊ တိုက်ရိုက် ပြန် scan လုပ်ထားသည်</span></div>
    {#if !opps.length}
      <p class="muted sm">လက်ရှိ ကိုယ်စားလှယ်လောင်းများထဲတွင် liquidity ရှိသော pool ≥2 ခုပါသော token မရှိပါ — ဈေးကွက်သည် တင်းကျပ်စွာ ချိတ်ဆက်ထားသည်။ နေ့စဉ် spread စစ်ထုတ်မှုသည် နောက် snapshot နှင့်အတူ update ဖြစ်မည်။</p>
    {:else}
      <table>
        <thead><tr><th>Jetton</th><th>ဝယ်ရန်</th><th>ရောင်းရန်</th><th class="r">Gross</th><th class="r">Net $</th><th class="r">Net %</th><th class="r">အရွယ်အစား</th></tr></thead>
        <tbody>
          {#each opps as o}
            <tr>
  ...
  <p class="muted foot">Net = spread − ဘက်နှစ်ဖက်စလုံး၏ slippage (CPMM၊ ဘက်တစ်ခုစီ s/y) − DEX fee 2×0.3% − gas ~$0.15၊
    အရွယ်အစားကို ပိုသေးသော half-reserve ၏ 50% အထိ scan ဖြင့် ရွေးချယ်သည်။ အပေါင်း net သည် ရှားပါးပြီး ခဏတာသာ ရှိသည် —
    အကောင်အထည်ဖော်မှုသည် atomic မဟုတ်ပါ (transaction နှစ်ခု)၊ ဘက်နှစ်ခုကြားတွင် ဈေးနှုန်း ပြောင်းသွားနိုင်သည်။ ကိုယ်စားလှယ်လောင်းများသည်
    snapshot ၏ နေ့စဉ် pool အချင်းချင်း spread (Track B) မှ ဖြစ်ပြီး GRAM ကို အမြဲ ပင်ထားသည်။ ဘဏ္ဍာရေးအကြံပေးချက် မဟုတ်ပါ။</p>
{/if}
