<script>
  ...
  const cats = [
    { id: 'all', label: 'အားလုံး' }, { id: 'meme', label: 'Memes' }, { id: 'defi', label: 'DeFi' },
...
<header class="hd">
  <div class="hd-top">
    <h1>ဈေးကွက်</h1>
    {#if meta.live}<span class="live"><span class="dot"></span>တိုက်ရိုက် · 60 စက္ကန့်</span>
    {:else}<span class="live off">နေ့စဉ်</span>{/if}
    {#if meta.updated}<span class="muted upd">မှတ်တမ်း {meta.updated}{meta.intraday ? ' +နေ့တွင်း' : ''} · {meta.snaps} ရက်{meta.live ? ' · တိုက်ရိုက်ဈေးနှုန်း' : ''}</span>{/if}
  </div>
  <div class="tabs">
    {#each cats as c}
...
{:else}
  ...
  <section class="bento">
    <div class="kpis">
      ...
      <div class="kpi"><div class="kl">အရောင်းအဝယ်ပမာဏ 24 နာရီ</div><div class="kv mono">{fmtUsd(kpi.vol)}</div></div>
      <div class="kpi"><div class="kl">TON Core အညွှန်းကိန်း 7 ရက်</div><div class="kv mono" class:good={kpi.index > 0} class:bad={kpi.index < 0}>{fmtPct(kpi.index)}</div></div>
      <div class="kpi"><div class="kl">Breadth (ကျယ်ပြန့်မှု)</div><div class="kv mono">{kpi.breadth ? `${kpi.breadth.above}/${kpi.breadth.total}` : '—'}</div></div>
    </div>
    <div class="card map">
      <div class="sec-title">ဈေးကွက်မြေပုံ · CORE၊ ဧရိယာ ≈ √mcap၊ အရောင် = 7 ရက်</div>
      <Treemap {rows} />
      <div class="movers">
  ...
  <!-- Zone 2: agent feed -->
  <section>
    <h2 class="sec-title">Agent စီးကြောင်း <span class="muted">· အချက်ပြမှုများမှ အလိုအလျောက်၊ တစ်နေ့ 3 ကြိမ်</span></h2>
    {#if agents.length}
      <div class="agrid">{#each agents as c}<AgentCard card={c} />{/each}</div>
    {:else}<div class="muted pad">အချက်ပြမှု မရှိပါ — ဈေးကွက် တည်ငြိမ်နေသည်။</div>{/if}
  </section>

  <!-- Zone 3: supply pressure -->
  <section>
    <h2 class="sec-title">ထောက်ပံ့မှု ဖိအား <span class="muted">· ထုတ်ဝေမှု / staking / ဝင်ရောက်မှု</span></h2>
    {#if unlocks.length}
      <div class="card">
        <table class="ut">
          <thead><tr><th>Jetton</th><th>ဖြစ်ရပ်</th><th class="r">ဖိအား</th><th class="r">$ သက်ရောက်မှု</th></tr></thead>
          <tbody>
            {#each unlocks as u}
        ...
        <div class="note muted">မှတ်ချက်: client-side တွင် တွက်ချက်နိုင်သည်များ (unstake ကာလများ၊ ထုတ်ဝေမှု) ကိုသာ ပြထားသည်။ team-cliff ရက်စွဲအတိအကျများမှာ ပြင်ပအချိန်ဇယား မရှိသဖြင့် n/a ဖြစ်သည်။</div>
      </div>
    {:else}<div class="muted pad">ထူးခြားသော ထောက်ပံ့မှုဖိအား မရှိပါ။</div>{/if}
  </section>

  <!-- Zone 4: market internals -->
  <section>
    <h2 class="sec-title">Jetton အားလုံး {cat !== 'all' ? '· ' + cats.find((c) => c.id === cat).label : ''}</h2>
