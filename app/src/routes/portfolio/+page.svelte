<script>
  ...
  async function analyse(a = addr) {
    ...
    catch (e) { err = 'မအောင်မြင်ပါ: ' + e.message; }
    busy = false;
  }
</script>

<svelte:head><title>ပို့ဖိုလီယို — TON Quant</title></svelte:head>
<header class="hd"><h1>ပို့ဖိုလီယို</h1><span class="muted">TON wallet မည်သည့်တစ်ခု၏ ပိုင်ဆိုင်မှုများ</span></header>

<div class="addbar">
  <input class="search" placeholder="Wallet လိပ်စာ (EQ… / UQ…)" bind:value={addr} onkeydown={(e) => e.key === 'Enter' && analyse()} />
  <button class="btn" onclick={() => analyse()} disabled={busy}>{busy ? '…' : 'ခွဲခြမ်းစိတ်ဖြာရန်'}</button>
</div>
...
{#if w}
  <section class="kpis">
    <div class="kc"><div class="kl">စုစုပေါင်း</div><div class="kv mono">{fmtUsd(w.total)}</div></div>
    <div class="kc"><div class="kl">TON</div><div class="kv mono">{w.ton.toFixed(2)}</div><div class="kd muted">{fmtUsd(w.tonValue)}</div></div>
    <div class="kc"><div class="kl">Jetton များ (ဈေးနှုန်းရှိသည်)</div><div class="kv mono">{w.holdings.length}</div></div>
  </section>
  {#if w.holdings.length}
    <div class="card tw">
      <table>
        <thead><tr><th>Jetton</th><th class="r">ပမာဏ</th><th class="r">ဈေးနှုန်း</th><th class="r">တန်ဖိုး</th><th class="r">ရာခိုင်နှုန်း</th></tr></thead>
        <tbody>
  ...
  {:else}<div class="muted pad">တန်ဖိုးရှိသော jetton မရှိပါ (spam airdrop များကို စစ်ထုတ်ထားသည်)။</div>{/if}
{:else if !busy}
  <div class="muted pad">Wallet လိပ်စာ ထည့်ပါ — TON နှင့် တန်ဖိုးအမှန်ရှိသော jetton များကို ပြပေးမည်။</div>
{/if}
