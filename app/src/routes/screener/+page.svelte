<script>
  ...
  const cats = [['all', 'အားလုံး'], ['meme', 'Memes'], ['defi', 'DeFi'], ['stable', 'Stables'], ['staking', 'Staking']];
  const sorts = [['vol24', 'Vol 24 နာရီ'], ['mcap', 'MCap'], ['liq', 'Liquidity'], ['d1', '24 နာရီ %']];
...
<svelte:head><title>စစ်ထုတ်ရှာဖွေ — TON Quant</title></svelte:head>

<header class="hd">
  <div class="hd-top"><h1>စစ်ထုတ်ရှာဖွေ</h1><span class="muted">Liquidity ရှိသော TON token အားလုံး · အလိုအလျောက်ရှာဖွေတွေ့ရှိမှု</span>{#if updated}<span class="muted upd">ပြင်ဆင်ချိန် {updated}{live ? ' · တိုက်ရိုက်ဈေးနှုန်း (60 စက္ကန့်)' : ''}</span>{/if}</div>
  <div class="tabs">{#each cats as [id, label]}<button class="tab" class:on={cat === id} onclick={() => (cat = id)}>{label}</button>{/each}</div>
</header>

{#if st === 'loading'}<div class="muted pad">token စာရင်း ရယူနေသည်…</div>
{:else if st === 'empty'}<div class="muted pad">universe.json ဗလာဖြစ်နေသည် — scripts/universe.py ကို run ပါ။</div>
{:else}
  <StaleBanner when={updated} what="token စာရင်းစုဆောင်းမှု" />
  <div class="ctrl">
    <input class="search" placeholder="ticker သို့မဟုတ် လိပ်စာဖြင့် ရှာရန်…" bind:value={q} />
    <div class="sorts">{#each sorts as [id, label]}<button class="chip" class:on={sort === id} onclick={() => (sort = id)}>{label}</button>{/each}</div>
    <span class="muted cnt">{view.length} ခု</span>
  </div>
  <div class="card tw">
    <table>
      <thead><tr><th>Jetton</th><th>အမျိုးအစား</th><th class="r">ဈေးနှုန်း</th><th class="r" title="နေ့စဉ်စုဆောင်းချိန်က 24 နာရီ ပြောင်းလဲမှု — တိုက်ရိုက် ပြန်မတွက်ပါ">24h*</th><th class="r">MCap</th><th class="r">Liquidity</th><th class="r">Vol 24 နာရီ</th></tr></thead>
      <tbody>
        {#each view as t}
          <tr>
            <td data-label="Jetton"><a href="{base}/token?a={t.addr}"><span class="sym">{t.sym}</span></a></td>
            <td data-label="အမျိုးအစား" class="muted">{t.cat}</td>
            <td data-label="ဈေးနှုန်း" class="r mono">{fmtUsd(t.price)}</td>
            <td data-label="24 နာရီ" class="r mono" class:good={t.d1 > 0} class:bad={t.d1 < 0}>{fmtPct(t.d1)}</td>
            <td data-label="MCap" class="r mono">{fmtUsd(t.mcap)}</td>
            <td data-label="Liquidity" class="r mono">{fmtUsd(t.liq)}</td>
            <td data-label="Vol 24 နာရီ" class="r mono">{fmtUsd(t.vol24)}</td>
          </tr>
        {/each}
      </tbody>
    </table>
