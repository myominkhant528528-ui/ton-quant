...
const STAKE_WINDOW = { staking: 'unstake ~36 နာရီ', stable: 'mint/redeem ဖွင့်ထားသည်' };
...
export function buildUnlocks(rows) {
  return rows
    .map((r) => {
      ...
      else if (r.growth && r.growth.pct > 3) { event = 'ထုတ်ဝေမှု/holder ဝင်ရောက်မှု'; pressure = Math.min(r.growth.pct, 50); }
