...
{#if !model}
  <div class="empty">မှတ်တမ်းများ စုဆောင်းနေသည် — snapshot အနည်းဆုံး 9 ရက် လိုအပ်သည်။</div>
{:else}
  <div class="wrap">
    <table class="ll">
      ...
      <tbody>
        {#if model.pairs.length}
          {#each model.pairs as p}
            <tr>
              <th class="rh">{model.sel[p.i].sym}</th><td class="m">ဦးဆောင် →</td><th class="rh">{model.sel[p.j].sym}</th>
              <td class="r" style="background:{corrColor(p.fwd)}">{p.fwd.toFixed(2)}</td><td class="r m">{p.rev.toFixed(2)}</td>
            </tr>
          {/each}
        {:else}
          <tr><td colspan="5" class="m left">အတိုင်းအတာကျော်လွန်သော လမ်းကြောင်းပါ lead-lag မရှိပါ — ခြင်းတောင်းတစ်ခုလုံး တပြိုင်နက်တည်း ရွေ့နေသည်။</td></tr>
        {/if}
      </tbody>
    </table>
  </div>
  <div class="note">အားအကောင်းဆုံး ဦးဆောင်သူ: <strong>{model.sel[model.lead].sym}</strong> (net {model.net[model.lead] >= 0 ? '+' : ''}{model.net[model.lead].toFixed(1)}) — ၎င်း၏ လှုပ်ရှားမှုများသည် ခြင်းတောင်းထက် ကြိုတင်ရွေ့သည်။ အဓိက လိုက်နာသူ: <strong>{model.sel[model.follow].sym}</strong> (net {model.net[model.follow].toFixed(1)}). နောက်ကျမှု 1 ရက်၊ ဒေတာ {model.L} ရက်။</div>
{/if}
