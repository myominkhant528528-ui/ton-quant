...
{#if !model}
  <div class="empty">မှတ်တမ်းများ စုဆောင်းနေသည် — token တစ်ခုလျှင် snapshot အနည်းဆုံး 6 ရက် လိုအပ်သည်။</div>
{:else}
  <div class="wrap">
    <table class="corr">
      <thead>
  ...
  {#if model.note}
    <div class="note">အဆက်စပ်ဆုံး: <strong>{model.note.pair[0]}–{model.note.pair[1]}</strong> (r {model.note.pv.toFixed(2)}). အကောင်းဆုံး diversifier: <strong>{model.note.div}</strong> (avg r {model.note.mv.toFixed(2)}). ဒေတာ {model.steps + 1} ရက်။</div>
  {/if}
{/if}
