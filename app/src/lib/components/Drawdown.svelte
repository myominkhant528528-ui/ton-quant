...
{#if !model}
  <div class="empty">မှတ်တမ်းများ စုဆောင်းနေသည် — snapshot အနည်းဆုံး 5 ရက် လိုအပ်သည်။</div>
{:else}
  <div class="wrap">
    <table class="dd">
      <thead><tr><th></th><th class="r">Drawdown</th><th>အကွာအဝေး (အနိမ့်→အမြင့်)</th><th class="r">တည်နေရာ</th></tr></thead>
      <tbody>
        {#each model as r}
          <tr>
  ...
  <div class="note">Drawdown အနက်ဆုံး: <strong>{model[0].sym}</strong> ({model[0].dd.toFixed(0)}% — 40 ရက် အမြင့်ဆုံးမှ)။ အမြင့်ဆုံးနှင့် အနီးဆုံး: <strong>{model[model.length - 1].sym}</strong> ({model[model.length - 1].dd >= -0.05 ? 'အမြင့်သစ်တွင်' : model[model.length - 1].dd.toFixed(0) + '% — အမြင့်ဆုံးမှ'}၊ တည်နေရာ {model[model.length - 1].pos.toFixed(0)}%)။ token {model.length} ခု။</div>
{/if}
