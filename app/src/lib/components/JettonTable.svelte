<script>
  ...
  const filters = [
    ...
    { id: 'movers', label: 'အရွေ့ကြီးသူများ', fn: (r) => Math.abs(r.d1 ?? 0) >= 5 },
    { id: 'wash', label: 'Wash', fn: (r) => r.volTvl > 3 },
    { id: 'liquid', label: 'Liquidity ကောင်း', fn: (r) => r.tvl > 50000 }
  ];
...
<div class="tw">
  <table>
    <thead><tr>
      <th>Jetton</th><th class="r">ဈေးနှုန်း</th><th class="r">24 နာရီ</th><th class="r">7 ရက်</th>
      <th class="r">MCap</th><th class="r">TVL</th><th class="r">Vol/TVL</th><th class="r">Holder 7 ရက်</th>
    </tr></thead>
    <tbody>
      {#each shown as r}
        <tr>
          <td data-label="Jetton"><a href="{base}/token?a={r.addr}"><span class="sym">{r.sym}</span> <span class="cat muted">{r.cat}</span></a></td>
          <td data-label="ဈေးနှုန်း" class="r mono">{fmtUsd(r.price)}</td>
          <td data-label="24 နာရီ" class="r mono" class:good={r.d1 > 0} class:bad={r.d1 < 0}>{fmtPct(r.d1)}</td>
          <td data-label="7 ရက်" class="r mono" class:good={r.d7 > 0} class:bad={r.d7 < 0}>{fmtPct(r.d7)}</td>
          ...
          <td data-label="Holder 7 ရက်" class="r mono" class:good={r.growth?.pct > 0.05} class:bad={r.growth?.pct < -0.05}>
...
{#if filtered.length > 10}
  <button class="more" onclick={() => (expanded = !expanded)}>{expanded ? 'ချုံ့ရန်' : `အားလုံးပြရန် (${filtered.length})`}</button>
{/if}
