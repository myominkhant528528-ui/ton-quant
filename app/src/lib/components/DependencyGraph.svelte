<script>
  ...
  const ST = { ok: 'လတ်ဆတ်', stale: 'ဟောင်းနေ', missing: 'ဖိုင်မရှိ', error: 'အမှား', unknown: 'ဒေတာမရှိ' };
  let hover = $state(null);
  const hv = $derived(hover ? { ...posOf[hover], key: hover, ...statusOf[hover] } : null);
  const ageTxt = (a) => (a == null ? '—' : a < 1 ? `${Math.round(a * 60)} မိနစ်` : `${a.toFixed(1)} နာရီ`);
</script>

<div class="wrap">
  <svg viewBox="0 0 {W} {H}" role="img" aria-label="ဒေတာရင်းမြစ်များ ဆက်စပ်မှု ဂရပ်" preserveAspectRatio="xMidYMid meet">
    {#each g.links as l}
  ...
  {#if hv}
    <div class="tip" style="left:{(hv.x / W) * 100}%; top:{(hv.y / H) * 100}%">
      ...
      <div class="st {hv.status}">{ST[hv.status] ?? hv.status}</div>
      <div class="meta">မှတ်တမ်း: {hv.count ?? '—'} · အသက်: {ageTxt(hv.age_h)} / ကန့်သတ်ချက် {hv.max_age_h ?? '—'} နာရီ</div>
