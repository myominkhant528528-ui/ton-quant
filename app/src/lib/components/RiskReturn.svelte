...
{#if !model}
  <div class="empty">မှတ်တမ်းများ စုဆောင်းနေသည် — token တစ်ခုလျှင် snapshot အနည်းဆုံး 12 ရက် လိုအပ်သည်။</div>
{:else}
  <svg viewBox="0 0 {model.W} {model.H}" class="rr">
    ...
    <text x={model.pl + model.iw / 2} y={model.H - 4} text-anchor="middle" font-size="10" fill={MUT}>နှစ်စဉ် volatility</text>
    <text x="13" y={model.pt + model.ih / 2} text-anchor="middle" font-size="10" fill={MUT} transform="rotate(-90 13 {model.pt + model.ih / 2})">နှစ်စဉ် ပြန်အမြတ်</text>
    {#each model.bubbles as b}
      <circle cx={b.x.toFixed(1)} cy={b.y.toFixed(1)} r={b.rr.toFixed(1)} fill={model.cc(b.sh)} fill-opacity="0.82" stroke="#5a6472" stroke-width="0.6">
        <title>{b.sym} — ပြန်အမြတ် {b.ann.toFixed(0)}% / vol {b.vol.toFixed(0)}% / Sharpe {b.sh.toFixed(2)}</title>
      </circle>
      <text x={(b.x + b.rr + 3).toFixed(1)} y={(b.y + 3).toFixed(1)} font-size="9" fill={LBL}>{b.sym}</text>
    {/each}
    <text x={model.W - model.pr} y={model.pt + 8} text-anchor="end" font-size="9" fill={MUT}>အရွယ်အစား = mcap</text>
  </svg>
  <div class="wrap">
    <table class="rt">
      <thead><tr><th></th><th class="r">နှစ်စဉ်ပြန်အမြတ်</th><th class="r">နှစ်စဉ် vol</th><th class="r">Sharpe</th></tr></thead>
      <tbody>
  ...
  <div class="note">Risk-adjusted အကောင်းဆုံး: <strong>{model.rk[0].sym}</strong> (Sharpe {model.rk[0].sh.toFixed(2)}). အဆိုးဆုံး: <strong>{model.rk[model.rk.length - 1].sym}</strong> (Sharpe {model.rk[model.rk.length - 1].sh.toFixed(2)}). token {model.n} ခု၊ snapshot ကာလအတွင်း နေ့စဉ်ပိတ်ဈေးများ၊ rf=0။ ပူဖောင်းဧရိယာ ∝ mcap။</div>
{/if}
